...
Restent à faire manuellement après le script (rappels affichés) : renseigner
`TOPIC_NTFY` dans le nouveau config, **créer le token dédié** (voir ci-dessous),
puis le poser avec `mettre_a_jour_tokens_ccw.ps1 -NomService CCW-Watcher-Scrabble
-RepDepot C:\CCW\Scrabble`.
