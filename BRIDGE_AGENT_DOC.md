...
~/Bridge_Agent/
  ...
  consignes/          — consignes injectées en tête de chaque issue (§12.1)
...
### 12.1 Consignes injectées — architecture à trois couches (issue #209)

En plus du `CONTEXTE.md` par projet (injecté dans le **prompt** CCL via
`FICHIER_CONTEXTE`, cf. §7), le bridge injecte automatiquement des **consignes**
directement dans le **corps de chaque issue**, au moment de sa création par
`app/issues.py` (`construire_body` → `_consignes_injectees`). Elles se placent
**entre le tableau d'en-tête et le corps rédigé par Claude Chat**. Trois couches,
de la plus générale à la plus spécifique, lues dans le dossier `consignes/` :
...
**Ordre d'injection final :** en-tête → globales → type (si présent) → projet
(si présent) → corps. Le TYPE est déduit par `watcher.deduire_type_issue`
(même logique que partout ailleurs : champ `| TYPE | … |` du corps, sinon
préfixe du titre `Chef :`/`Ouvrier :`), donc le fichier attendu est
`consignes/type_<type>.md`. Vaut en **mono-issue comme en mode lot** : chaque
bloc `#Titre:` passe par `construire_body` avec son propre titre/corps/projet,
donc son propre TYPE.
...
**Garde-fous (aucune création d'issue ne doit échouer à cause des consignes) :**
- fichier `type_*`/`projet_*` **absent** → comportement **normal**, aucune
  ...
  anomalie) ;
- `globales.md` **introuvable** → `logging.warning` clair, mais la création
  d'issue se poursuit sans jamais échouer pour cette raison seule.
...
**⚠️ Contrainte d'exécution impérative** : le chef doit accomplir la
...
aucune reprise possible après qu'une issue a été répondue/fermée : ne jamais
utiliser un mécanisme d'attente différée, de tâche en arrière-plan ou de
« je répondrai plus tard ». Si une attente est nécessaire, boucler
(`sleep` + `gh issue view`) DANS la même exécution.

> **Injection automatique (issue #209).** Ce rappel n'est plus à recopier
> manuellement dans le corps d'une issue chef : il est **injecté
> automatiquement** dans le corps de toute issue de TYPE `chef` via
> `consignes/type_chef.md` (architecture à trois couches, §12.1). Le texte
> ci-dessus reste dans cette section comme référence documentaire, mais Claude
> Chat n'a plus besoin de le dupliquer à la main dans chaque issue.
