...
- #625→#639 (refonte web) : socle de modules ES dans `static/js/socle/`
  (store/api/sse/toasts/dom/persistance/pont), et modules par fonctionnalité
  sortis d'app.js (onglets, resultats — `/stream` + badges + lignes fichier
  reçu/refusé, onglet « Résultats inbox » fusionné dans Résultats #639,
  panneau latéral, resultats_coches — case « traité/lu » serveur #636)
  communiquant entre eux par le store. Détail : `ARCHITECTURE.md §6`.
