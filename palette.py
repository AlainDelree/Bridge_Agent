...
# non affichée — voir la priorité dans couleurs_utilisees() (nouveau_projet.py)
# et dans COULEURS_PROJET de app.js (qui DOIT rester en synchro avec ce
# dictionnaire).
...
# disponibles pour un futur projet (voir couleurs_utilisees()/
# couleurs_disponibles() dans nouveau_projet.py, et le gain concret vérifié
# en clôture d'issue). Les inclure ICI avec la valeur grise a été tenté puis
# abandonné : ce dictionnaire est aussi passé en couleurs_a_eviter à
# generer_palette(), qui raisonne en angle de teinte Lab (_teinte_lab) — un
# gris (saturation 0) a un a*/b* quasi nul, donc un angle atan2(0,0) dégénéré
# à 0°, qui entre en collision avec l'exclusion de teinte prévue pour les
# rouges et fait échouer l'assertion de generer_palette (vérifié
# concrètement, pas supposé). Leur couleur d'affichage grise vit donc
# UNIQUEMENT dans COULEURS_PROJET côté app.js (source de vérité pour
# l'affichage), pas ici (source de vérité pour la réservation de couleurs
# actives) — voir COULEUR_PROJET_INACTIF ci-dessous et §"Couleur d'accent des
# projets" de BRIDGE_AGENT_DOC.md pour la procédure de recyclage à réutiliser
# pour un futur projet mis à l'arrêt.
...
# exclues de la proposition (voir couleurs_disponibles dans nouveau_projet.py).
# Hex #RRGGBB en MAJUSCULES, comparés sans tenir compte de la casse.
#
...
# le fait déjà (nouveau_projet.py) — sans ça, generer_palette() ne
# garantirait la distance/teinte qu'ENTRE les couleurs qu'elle génère, pas
# vis-à-vis des 11 couleurs gelées en dur. C'est exactement ce trou qui avait
# laissé passer la collision actualise/gestionmail : gestionmail a pris une
# couleur de cette même palette, valide par rapport aux autres couleurs
# générées, mais jamais vérifiée par rapport à actualise (gelée à part).
# Avec ce paramètre, toute couleur encore proposée à un futur projet est
# garantie distincte de TOUS les projets existants, pas seulement des autres
