...
function basculerOnglet(nom) {
  ...
  if (nom === 'resultats') {
    // Refonte étape 3 (issue #627) : la liste Résultats est désormais pilotée
    // par le store et le SSE (static/js/resultats.js). L'ACTIVATION de l'onglet
    // ne déclenche PLUS aucun rechargement réseau — un chargement initial unique
    // (première activation) + des mises à jour ciblées par événement /stream + le
    // ↻ explicite suffisent. Le canal /stream tourne en permanence (ouvert par
    // resultats.initialiser() au chargement de la page). Le panneau latéral
    // (hors périmètre #627) reste géré ici par l'ancien code.
    if (window.Bridge && window.Bridge.resultats) window.Bridge.resultats.onActiverOnglet();
    demarrerPanneauLateral();
  } else {
    if (window.Bridge && window.Bridge.resultats) window.Bridge.resultats.onDesactiverOnglet();
