...
| `../tests/actions_ligne.test.js` | Actions sur la ligne d'une issue OUVERTE (issue #641, correctif #642) : `prefixeInformatifLigneOuverte` (needs-human cliquable/mode_write redevenu informatif/aucun préfixe selon les labels), `afficherIconeInterruption` (icône dédiée basée sur `timing.debut` — en cours lecture/écriture → true, en file/needs-human/timing absent → false), son par issue (#630/#637, réutilisées telles quelles) et son cache par projet (`fusionnerSonsProjet`/`sonConnuDansCache`). |
